...
class EOQAlgorithm(StockAlgorithm):
    ...
    def calculate_total_cost(self, annual_demand: float, ordering_cost: float, 
                            holding_cost: float, order_quantity: float) -> Tuple[float, float, float]:
        """
...
class ReorderPointAlgorithm(StockAlgorithm):
    ...
    def calculate_from_annual(self, annual_demand: float, lead_time_days: int, 
                             safety_stock: float = 0, working_days: int = 365) -> float:
        """
...
class SafetyStockAlgorithm(StockAlgorithm):
    ...
    def calculate(self, service_level_z: float, demand_std_dev: float, lead_time_days: int) -> float:
        """
